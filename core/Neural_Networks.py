...
def PriceFrame_to_ImgArray(prices):
...
class Single_Input_Convolutional(SimpleNet1):
	def build_model(self, input_size=(30,), nb_actions=2):
		...
		img_layer = keras.layers.Reshape(input_size)(input_layer)
		conv_layer = keras.layers.Conv2D(64, (3, 3))(img_layer)
		max_pooling = keras.layers.MaxPool2D()(conv_layer)
		hidden_layer = keras.layers.Flatten()(max_pooling)
		...
		output_layer = keras.layers.Dense(nb_actions, activation="sigmoid")(hidden_layer)
	...
	def specialized_data_funnel(self, env=gym.make("forex-v0")):
		...
		img_np_array = np.asarray(PriceFrame_to_ImgArray(prices_df.to_numpy()))
	...
	def specialized_observation(self, env=gym.make("forex-v0")):
		...
		img_array = PriceFrame_to_ImgArray(prices=env.prices[env._current_tick - env.window_size: env._current_tick])
		return img_array
